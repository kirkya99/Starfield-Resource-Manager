...
import { columns } from 'src/typescript/CraftableResource'
...
const deleteRow = () => {
  if (toBeDeletedName.value) {
    storeManager.craftableResourceListStore.removeCraftableResource(toBeDeletedName.value)
    Notify.create({
      type: 'positive',
      message: `${toBeDeletedName.value} removed from craftable resource list`,
      actions: [
        {
          icon: 'close',
          color: 'white'
    ...
    Notify.create({
      type: 'negative',
      message: 'Error while removing: Failed to remove item from craftable resource list',
      actions: [
        {
          icon: 'close',
          color: 'white'
...
<template>
  <div class="row justify-center q-gutter-y-sm">
    ...
    <div class="col-md-9 col-xs-12 q-mt-md">
      <q-table flat bordered :rows="storeManager.craftableResourceListStore.craftableResourceList" :columns="columns" row-key="name"
  ...
  <q-dialog v-model="deleteDialogOpened" persistent>
    <q-card>
      <q-card-section avatar class="row items-center">
        ...
        <span class="q-ml-sm text-primary">{{ `Remove ${toBeDeletedName} from craftable resource list?` }}</span>
      </q-card-section>
